package liquibasedemo.data;

// Generated 2015-05-04 22:44:44 by Hibernate Tools 3.4.0.CR1

import javax.persistence.Column;
import javax.persistence.Embeddable;

/**
 * Movies2writersId generated by hbm2java
 */
@Embeddable
public class Movies2writersId implements java.io.Serializable {

    private int movieid;
    private int writerid;
    private String addition;

    public Movies2writersId() {
    }

    public Movies2writersId(int movieid, int writerid) {
        this.movieid = movieid;
        this.writerid = writerid;
    }

    public Movies2writersId(int movieid, int writerid, String addition) {
        this.movieid = movieid;
        this.writerid = writerid;
        this.addition = addition;
    }

    @Column(name = "movieid", nullable = false)
    public int getMovieid() {
        return this.movieid;
    }

    public void setMovieid(int movieid) {
        this.movieid = movieid;
    }

    @Column(name = "writerid", nullable = false)
    public int getWriterid() {
        return this.writerid;
    }

    public void setWriterid(int writerid) {
        this.writerid = writerid;
    }

    @Column(name = "addition", length = 1000)
    public String getAddition() {
        return this.addition;
    }

    public void setAddition(String addition) {
        this.addition = addition;
    }

    public boolean equals(Object other) {
        if ((this == other))
            return true;
        if ((other == null))
            return false;
        if (!(other instanceof Movies2writersId))
            return false;
        Movies2writersId castOther = (Movies2writersId) other;

        return (this.getMovieid() == castOther.getMovieid())
                && (this.getWriterid() == castOther.getWriterid())
                && ((this.getAddition() == castOther.getAddition()) || (this.getAddition() != null
                && castOther.getAddition() != null && this.getAddition().equals(castOther.getAddition())));
    }

    public int hashCode() {
        int result = 17;

        result = 37 * result + this.getMovieid();
        result = 37 * result + this.getWriterid();
        result = 37 * result + (getAddition() == null ? 0 : this.getAddition().hashCode());
        return result;
    }

}
